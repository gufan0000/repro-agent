...
- **Finding your way around.** A version string is usually not a tag: `3.3.1.0` returns 404 where `v3.3.1.0` succeeds. On GitHub, `https://api.github.com/repos/<owner>/<repo>/tags` lists the real tags and `https://api.github.com/repos/<owner>/<repo>/git/trees/<ref>?recursive=1` lists every file at one ref in a single request. Use that API to look things up, not to read: it allows 60 anonymous requests an hour per address, while raw file fetches are not limited that way. If the API is unavailable — rate-limited, or a host that has none — you do not need it: fetching `raw…/<candidate-ref>/README.md` and getting a 200 proves that ref exists. Any other host works the same way; open one file in its web UI and its raw URL shape is right there.
- **Which revision to read — work down, stop at the first rung that holds.** Every rung beats reading nothing. What matters is not that you read the exact build, but that the report says which rung you used.
  1. `project.commit`, or a tag matching `project.version`. Try the variants before concluding there is no match — for `3.3.1.0` that is `v3.3.1.0`, `3.3.1.0`, `v3.3.1`, `3.3.1`, `release-3.3.1`. A full or seven-character SHA works anywhere a tag does.
...
1. The source or documentation shows how the software is **supposed** to behave — at the installed revision, or at another one plus the Phase B cross-check that ties it to this machine. A claim read only at the default branch, with nothing on this machine confirming that code is in the running build, does not satisfy this. It is enough to write a report; it is not enough to change anything.
