...
`0.5.1`，协议 `1.0`。规范、CLI、离线页面和两个适配器都已完成，79 个测试（`npm test`）加 13 个真实浏览器测试（`npm run test:browser`）覆盖，在 Linux、macOS、Windows 上跑 Node 20 和 22。

已经对着一个真实缺陷做过两次盲测，两个不同的模型。全新的 agent 只拿到一份任务文件和「开始」两个字，两次都在几分钟内定位到根因、给出 `文件:行号` 引用、排除备选假设、遵守策略禁令且一个字节都没改 ——
**[现场报告 001](docs/field-reports/2026-08-14-tasklite-crlf.md)**（Claude Sonnet 4.5）和
**[002](docs/field-reports/2026-08-16-gemini-fabricated-provenance.md)**（Gemini 3.7 Flash）。两次合计暴露出协议本身的六个漏洞 —— 现在都已补上。

002 更有价值，因为它抓到协议在自己的主张上翻了车：诊断完全正确，却给出了一条指向它从未访问过的仓库的引用 —— 由工具调用记录证明全程零次网络请求。失败经过和修复后的复测都写在那份报告里。

如实说明局限：

- 那是**两次**运行、两个模型、一个缺陷，而且是在埋这个 bug 的人自己的机器上跑的。它是个下限，不是度量。**模型在真实场景下遵守这套协议有多稳定，仍然是未知数**，真实使用反馈依然是最有价值的贡献。
- 两次运行源码都本来就在磁盘上，所以「本地什么都没有、只能靠远端」那条路径从未被真正走过。那是协议里测试最少、又恰好最近才大改过的部分。
