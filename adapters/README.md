...
These are the *generic* versions, with no project profile baked in. They work as-is: an agent that reads one of them will follow the protocol using local evidence and public documentation. To get a version that knows your software, run `npx bugbridge build` with your own profile.
...
| `workbuddy/bugbridge-doctor/SKILL.md` | Zip the `bugbridge-doctor/` folder and upload it as a skill in WorkBuddy or OpenClaw. |
| `workbuddy/bugbridge-doctor/SKILL.zh-CN.md` | Same, Chinese. Rename to `SKILL.md` inside the package before zipping. |
